...
class HiddenUsersViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, HiddenUserDelegate {
    ...
    func unblockUser(senderTag: Int) {
        ...
        AlertShow.confirm(inpView: self, titleStr: "Unblock \(blockedUser.name)?", messageStr: "\(blockedUser.name)'s trees, photos and comments will be visible to you again.", dismissIfNo: false, completion: {
